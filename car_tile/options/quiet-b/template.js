...
const rowH = (glyph, txt, col, house) => `<div style='display:flex;align-items:center;justify-content:flex-end;gap:5px;font-size:11.5px;line-height:1.3;color:${col};white-space:nowrap;font-variant-numeric:tabular-nums;${stale && !house ? 'opacity:0.45;' : ''}'>${glyph}<span>${txt}</span></div>`;
...
let rows = '';
if (atHome && !stale) { const c = gOpen ? AMB : dimCol; rows += rowH(gGlyph(c, gOpen), gOpen ? 'Home · ' + gTxt.charAt(0).toLowerCase() + gTxt.slice(1) : 'Home', c, gOpen); }
else if (placeTxt && !stale) { rows += rowH(st === 'driving' ? nav(dimCol) : pin(dimCol), placeTxt, dimCol); if (gOpen) { rows += rowH(gGlyph(AMB, true), gTxt, AMB, true); } }
else if (gOpen) { rows += rowH(gGlyph(AMB, true), gTxt, AMB, true); }
if (!trkS) { const geo = S('sensor.tesla_geofence'); if (geo && !['unknown','unavailable','none',''].includes(String(geo).trim().toLowerCase())) { rows += rowH(pin(dimCol), geo, dimCol); } }
...
const info = `<div style='display:flex;flex-direction:column;gap:4px;opacity:1;'>${rows}</div>`;
